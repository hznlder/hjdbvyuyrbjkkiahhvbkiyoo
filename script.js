...
class CodeForge {
    ...
    setupAuthEventListeners() {
        ...
        if (authSwitch) {
            authSwitch.addEventListener('click', () => this.switchAuthMode());
        }
        if (authForm) {
        ...
        if (savePreferences) {
            savePreferences.addEventListener('click', () => this.saveUserPreferences());
        }

        // Robot check
        const notRobotBtn = document.getElementById('not-robot-btn');
        if (notRobotBtn) {
            notRobotBtn.addEventListener('click', () => this.incrementVisitorCount());
        }

        // Modal overlay clicks
        document.addEventListener('click', (e) => {
            if (e.target.classList.contains('modal-overlay')) {
                if (this.isAuthModalOpen) this.hideAuthModal();
                if (this.isProfileModalOpen) this.hideProfileModal();
            }
        });
    }
    ...
    showAuthModal(mode = 'login') {
        ...
        if (mode === 'login') {
            ...
            switchText.innerHTML = 'Don\'t have an account? <button type="button" id="auth-switch" class="auth-switch-btn">Sign up</button>';
            confirmGroup.classList.add('hidden');
        } else {
            title.textContent = 'Sign Up';
            submitText.textContent = 'Sign Up';
            switchText.innerHTML = 'Already have an account? <button type="button" id="auth-switch" class="auth-switch-btn">Login</button>';
            confirmGroup.classList.remove('hidden');
        }

        // Re-attach event listener for the new switch button
        const newAuthSwitch = document.getElementById('auth-switch');
        if (newAuthSwitch) {
            newAuthSwitch.addEventListener('click', () => this.switchAuthMode());
        }
    ...
    switchAuthMode() {
        ...
        this.hideAuthModal();
        setTimeout(() => this.showAuthModal(newMode), 100);
    }
    ...
    showProfileModal() {
        ...
        if (this.currentUser) {
            ...
            profileJoinDate.textContent = `Member since ${new Date(joinDate).toLocaleDateString()}`;
        }
    ...
    loadPreferencesIntoModal() {
        // Load favorite games
        const favGenshin = document.getElementById('fav-genshin');
        const favHsr = document.getElementById('fav-hsr');
        const favZzz = document.getElementById('fav-zzz');

        if (favGenshin) favGenshin.checked = this.userPreferences.favoriteGames.includes('genshin');
        if (favHsr) favHsr.checked = this.userPreferences.favoriteGames.includes('hsr');
        if (favZzz) favZzz.checked = this.userPreferences.favoriteGames.includes('zzz');

        // Load notification preferences
        const notifyNewCodes = document.getElementById('notify-new-codes');
        const notifyFavorites = document.getElementById('notify-favorites');

        if (notifyNewCodes) notifyNewCodes.checked = this.userPreferences.notifications.newCodes;
        if (notifyFavorites) notifyFavorites.checked = this.userPreferences.notifications.favoritesOnly;
    }
    ...
    async saveUserPreferences() {
        ...
        // Get favorite games
        const favoriteGames = [];
        if (document.getElementById('fav-genshin')?.checked) favoriteGames.push('genshin');
        if (document.getElementById('fav-hsr')?.checked) favoriteGames.push('hsr');
        if (document.getElementById('fav-zzz')?.checked) favoriteGames.push('zzz');

        // Get notification preferences
        const notifications = {
            newCodes: document.getElementById('notify-new-codes')?.checked || false,
            favoritesOnly: document.getElementById('notify-favorites')?.checked || false
        };

        this.userPreferences = { favoriteGames, notifications };
        ...
        try {
            ...
            if (userDoc.exists) {
                ...
                this.userPreferences = {
                    ...
                    notifications: userData.notifications || { newCodes: false, favoritesOnly: false }
                };
            }
        } catch (error) {
    ...
    async initializeFirebaseMessaging() {
        ...
        messaging.onMessage((payload) => {
            this.showNotification(payload.notification.body, 'info');
        });
    }
    ...
    async loadVisitorCount() {
        try {
            const counterDoc = await db.collection('counters').doc('visitors').get();
    ...
    async incrementVisitorCount() {
        try {
            await db.collection('counters').doc('visitors').update({
                count: firebase.firestore.FieldValue.increment(1)
            });
            
            // Update UI
            const currentCount = parseInt(document.getElementById('visitor-count').textContent.replace(/,/g, ''));
            document.getElementById('visitor-count').textContent = (currentCount + 1).toLocaleString();
            
            this.showNotification('Thank you for verifying!', 'success');
            
            // Disable button temporarily
            const btn = document.getElementById('not-robot-btn');
            btn.disabled = true;
            btn.innerHTML = '<i class="fas fa-check"></i><span>Verified</span>';
            
            setTimeout(() => {
                btn.disabled = false;
                btn.innerHTML = '<i class="fas fa-check"></i><span>I am not a robot</span>';
            }, 5000);
        } catch (error) {
            // If document doesn't exist, create it
            try {
                await db.collection('counters').doc('visitors').set({ count: 1 });
                document.getElementById('visitor-count').textContent = '1';
                this.showNotification('Thank you for verifying!', 'success');
            } catch (createError) {
                this.showNotification('Error updating counter', 'error');
            }
        }
    }
